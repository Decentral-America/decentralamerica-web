---
title_es: Quién guarda el registro
title_en: Who holds the record
description_es: Una institución no puede ser a la vez el sujeto de un registro y su custodio. No es una cuestión de confianza. Es una cuestión de quién puede demostrar algo sobre el pasado.
description_en: An institution cannot be both the subject of a record and its keeper. This is not a question of trust. It is a question of who is able to prove anything about the past.
date: 2026-08-20
type: articulo
slug: quien-guarda-el-registro
tags: custodia, memoria publica
---

--- es ---

Un registro público tiene dos propiedades que casi nunca se distinguen. Una es que la información esté disponible. La otra es que la información de ayer siga siendo la de ayer.

La primera se resuelve con un portal. La segunda no la resuelve nada de lo que hoy opera en la región.

## El problema no es el acceso

Casi todos los países de Centroamérica publican sus compras públicas en línea. Los portales existen, funcionan, y en general contienen lo que dicen contener. El problema aparece un día después.

Cuando un archivo se corrige, la versión corregida se publica en la misma dirección y la anterior desaparece. No queda una versión previa que consultar, ni un aviso de que hubo un cambio. Desde afuera, un portal que nunca tocó un dato y uno que reescribió [catorce meses ya cerrados](/publicaciones/cinco-republicaciones-catorce-meses) se ven exactamente igual.

Eso no convierte a nadie en culpable. Convierte al registro en inservible como prueba, que es distinto y peor. Un archivo que puede cambiar sin dejar rastro no sostiene ninguna afirmación sobre el pasado, ni siquiera una afirmación favorable a quien lo publica.

## Por qué el custodio no puede ser el sujeto

La objeción habitual es que basta con que la institución se comprometa a no reescribir. Un compromiso es una política, y una política dura lo que dura la administración que la firmó.

El punto más incómodo es otro. Aunque cada funcionario actúe con integridad absoluta, la institución sigue siendo el único testigo de su propio pasado. Si el archivo cambió, es su palabra. Si no cambió, también es su palabra. Nadie desde afuera puede separar los dos casos, y por lo tanto la institución honesta y la que no lo es tienen la misma capacidad de demostrarlo, que es ninguna.

Eso perjudica primero a la institución honesta. Un ministerio que hizo bien su trabajo durante ocho años termina el periodo sin poder probarlo, porque la única evidencia disponible es el archivo actual, y el archivo actual es indistinguible de uno reescrito ayer. La ausencia de memoria no protege al que tiene algo que esconder. Protege igual a todos, y por eso no le sirve a nadie.

## Qué hace falta en su lugar

Que la copia esté en manos de organizaciones sin interés en lo que la copia dice. Universidades, colegios profesionales y medios: instituciones con horizonte largo, sin rendimiento financiero en juego, y sin nada que ganar si el registro favorece a un gobierno ni nada que perder si lo contradice.

No se trata de que sean incorruptibles. Se trata de que son varias, de que no responden a la misma autoridad, y de que reescribir el pasado exigiría que todas lo hicieran a la vez y en la misma dirección. Ese requisito es mucho más difícil de cumplir que la buena voluntad sostenida de una sola oficina.

> Una memoria que sobrevive a un cambio de gobierno no puede estar en manos de ningún gobierno.

La frase no es una acusación. Describe dónde tiene que estar guardado un archivo para que siga sirviendo después de que cambien las personas que lo escribieron.

--- en ---

A public record has two properties that are almost never told apart. One is that the information is available. The other is that yesterday's information is still yesterday's information.

A portal solves the first. Nothing currently running in the region solves the second.

## The problem is not access

Almost every country in Central America publishes its public procurement online. The portals exist, they work, and they generally contain what they say they contain. The problem shows up a day later.

When a file is corrected, the corrected version is published at the same address and the previous one disappears. There is no earlier version to consult and no notice that anything changed. From outside, a portal that never touched a figure and one that rewrote [fourteen already-closed months](/publicaciones/cinco-republicaciones-catorce-meses) look exactly the same.

That makes nobody guilty. It makes the record useless as evidence, which is a different problem and a worse one. A file that can change without leaving a trace supports no claim about the past, including a claim that favours whoever publishes it.

## Why the keeper cannot be the subject

The usual objection is that the institution can simply commit to not rewriting anything. A commitment is a policy, and a policy lasts as long as the administration that signed it.

The harder point is a different one. Even if every official acts with complete integrity, the institution remains the only witness to its own past. If the file changed, that is its word. If it did not change, that is also its word. Nobody outside can separate the two cases, so the honest institution and the dishonest one have the same ability to prove it, which is none.

The honest institution is the one that loses first. A ministry that did its job well for eight years reaches the end of the term unable to show it, because the only available evidence is the current file, and the current file is indistinguishable from one rewritten yesterday. Missing memory does not protect whoever has something to hide. It protects everyone equally, which is why it is worth nothing to anyone.

## What is needed instead

The copy has to sit with organizations that have no interest in what the copy says. Universities, professional colleges and newsrooms: institutions with a long horizon, no financial return at stake, nothing to gain if the record favours a government and nothing to lose if it contradicts one.

The point is not that they are incorruptible. The point is that there are several of them, that they do not answer to the same authority, and that rewriting the past would require all of them to do it at once and in the same direction. That is a far harder condition to meet than the sustained goodwill of a single office.

> A memory that outlives a change of government cannot be held by any government.

The line is not an accusation. It describes where a file has to be kept if it is still going to be worth anything after the people who wrote it are gone.
